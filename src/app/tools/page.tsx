...
const RARE_EARTH_PRICES = [
  { symbol: "La", name: "Lanthanum", price: 4.5, change: +2.3, unit: "$/kg" },
  { symbol: "Ce", name: "Cerium", price: 4.2, change: -1.2, unit: "$/kg" },
  { symbol: "Nd", name: "Neodymium", price: 152.5, change: +5.8, unit: "$/kg" },
  {
    symbol: "Pr",
    name: "Praseodymium",
    price: 128.0,
  ...
  { symbol: "Tb", name: "Terbium", price: 1650.0, change: -2.1, unit: "$/kg" },
  { symbol: "Eu", name: "Europium", price: 35.0, change: +0.5, unit: "$/kg" },
  { symbol: "Y", name: "Yttrium", price: 32.5, change: +1.8, unit: "$/kg" },
  { symbol: "Sm", name: "Samarium", price: 8.2, change: -0.8, unit: "$/kg" },
  { symbol: "Gd", name: "Gadolinium", price: 45.0, change: +2.1, unit: "$/kg" },
];
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/10">
              ...
              <th className="text-center py-3 px-2 text-yellow-500/80 font-medium">
                Low
              </th>
              <th className="text-center py-3 px-2 text-yellow-500 font-medium">
                Medium
              </th>
              <th className="text-center py-3 px-2 text-green-500 font-medium">
                High
              </th>
              <th className="text-center py-3 px-4 text-geo-gold font-medium">
                Discovery
              </th>
            </tr>
          </thead>
          <tbody>
            {items.map((item, idx) => (
              ...
              >
                ...
                <td className="text-center py-2.5 px-2 text-gray-400">
                  {item.units}
                </td>
                <td className="text-center py-2.5 px-2 text-gray-300">
                  {item.lowMax || item.lowMin}
                </td>
                <td className="text-center py-2.5 px-2 text-gray-300">
                  {item.medMin} - {item.medMax}
                </td>
                <td className="text-center py-2.5 px-2 text-gray-300">
                  {item.highMin}
                </td>
                <td className="text-center py-2.5 px-4 text-geo-gold font-medium">
                  {item.discovery}
                </td>
              </tr>
            ))}
...
export default function ToolsPage() {
  ...
  const [alerts, setAlerts] = useState<
    Array<{ metal: string; threshold: string; direction: string }>
  >([]);
  ...
  const calculateResourceEstimation = () => {
    ...
    setResourceResult({
      contained: containedMetal.toLocaleString(undefined, {
        maximumFractionDigits: 2,
      }),
      recoverable: recoverableMetal.toLocaleString(undefined, {
        maximumFractionDigits: 2,
      }),
    ...
    <main className="min-h-screen bg-gradient-to-br from-geo-dark to-black text-white pt-32 pb-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-12">
          ...
          >
            <span className="text-geo-gold text-sm font-medium tracking-widest uppercase">
              TOOLS
            </span>
            <h1 className="text-4xl md:text-5xl font-bold mt-2">
              Market Analysis Tools <span className="text-geo-gold">&</span>{" "}
              Calculators
            </h1>
            <p className="text-gray-400 mt-4 max-w-2xl">
              Informational use only. Not financial advice. Results are
              estimates based on assumptions and may differ from real-world
              outcomes.
            </p>
          </motion.div>
        </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-black/50 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-gray-500 focus:outline-none focus:border-geo-gold transition-colors"
                  />
                </div>
                <div>
                  <label className="text-sm text-gray-400 block mb-1.5">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={tonneValue}
                    onChange={(e) => handleTonneChange(e.target.value)}
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-black/50 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-geo-gold transition-colors"
                  >
        ...
        >
          ...
          <div className="bg-white/5 rounded-xl border border-white/10 p-6">
            <p className="text-gray-400 text-sm mb-6">
              Calculate equivalent grades across different commodities to
              compare deposit values accurately.
            </p>
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <button
                  onClick={calculateEquivalentGrade}
              ...
              {RARE_EARTH_PRICES.map((element) => (
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-geo-gold font-bold text-lg">
                      {element.symbol}
                    </span>
                    <span
                      className={`text-xs font-medium ${element.change >= 0 ? "text-green-400" : "text-red-400"}`}
                    >
                      {element.change >= 0 ? "+" : ""}
                      {element.change}%
                    </span>
          ...
          <div className="bg-white/5 rounded-xl border border-white/10 p-6">
            ...
            {npvResult && (
              <div className="mt-6 p-4 bg-geo-gold/10 border border-geo-gold/30 rounded-lg">
                ...
                <div
                  className={`text-2xl font-bold ${parseFloat(npvResult) >= 0 ? "text-green-400" : "text-red-400"}`}
                >
                  ${parseFloat(npvResult).toLocaleString()}
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  {parseFloat(npvResult) >= 0
                    ? "Project is potentially viable"
                    : "Project may not be viable at this discount rate"}
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-white/5 rounded-xl border border-white/10 p-6">
            ...
            <div className="grid md:grid-cols-4 gap-4">
              <div>
                ...
                <select
                  value={alertMetal}
                  onChange={(e) => setAlertMetal(e.target.value)}
                  className="w-full bg-black/50 border border-white/20 rounded-lg px-4 py-3 text-white focus:outline-none focus:border-geo-gold transition-colors"
                >
                ...
                {alerts.map((alert, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-geo-gold font-bold">
                        ${alert.threshold}
                      </span>
                    </div>
                    <button
          ...
          <div className="bg-white/5 rounded-xl border border-white/10 p-6">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-5 gap-4">
              ...
              <div>
                ...
                <button
                  onClick={calculateResourceEstimation}
                  className="w-full bg-geo-gold text-black font-bold py-3 rounded-lg hover:bg-yellow-500 transition-colors"
                >
                  Estimate
                </button>
            ...
            {resourceResult && (
              <div className="mt-6 grid md:grid-cols-3 gap-4">
                ...
                <div className="p-4 bg-geo-gold/10 border border-geo-gold/30 rounded-lg">
                  ...
                  <div className="text-xl font-bold text-geo-gold">
        ...
        >
          ...
          <div className="bg-white/5 rounded-xl border border-white/10 p-6">
            ...
            <GradeTable
              title={GRADE_DATA.baseMetals.title}
              items={GRADE_DATA.baseMetals.items}
            />
            <GradeTable
              title={GRADE_DATA.preciousMetals.title}
              items={GRADE_DATA.preciousMetals.items}
            />
          </div>
        </motion.section>
      </div>
