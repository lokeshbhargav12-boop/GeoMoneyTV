...
import {
    ArrowLeft, Zap, Globe2, Factory,
    Battery, Sun, Wind, Droplets, Atom, Flame,
    Gauge, AlertTriangle, Shield, ChevronRight,
    BarChart3, Fuel, Warehouse, Cable,
    TrendingUp, TrendingDown, Loader2,
    Server, RefreshCw, AlertCircle, Container, Truck, Layers, CheckCircle2, XCircle,
} from 'lucide-react'
...
const TECH_CATS = [{ id: 'all', label: 'All Technologies' }, { id: 'fossil', label: 'Fossil Fuels' }, { id: 'renewable', label: 'Renewables' }, { id: 'storage', label: 'Storage' }, { id: 'grid', label: 'Grid' }]
...
const CONSTRAINTS = [
    { icon: Gauge, label: 'Physical Capacity', sev: 'high', desc: 'Infrastructure at or near nameplate capacity.' },
    { icon: Cable, label: 'Interconnection Limits', sev: 'high', desc: 'Queue delays for new generation and storage.' },
    { icon: AlertCircle, label: 'Permitting', sev: 'medium', desc: 'Regulatory processes delaying development.' },
    { icon: RefreshCw, label: 'Maintenance', sev: 'medium', desc: 'Outages reduce capacity during critical periods.' },
    { icon: Sun, label: 'Weather Exposure', sev: 'variable', desc: 'Extreme weather can disrupt operations.' },
    { icon: Truck, label: 'Logistics', sev: 'medium', desc: 'Rail, truck, barge, and port constraints.' },
    { icon: RefreshCw, label: 'Equipment Supply', sev: 'medium', desc: 'Transformer, turbine, and cable shortages.' },
    { icon: Warehouse, label: 'Storage Limits', sev: 'variable', desc: 'Depleted buffers reduce system flexibility.' },
]
            ...
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                ...
                <div className="mb-12">
                    ...
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                        ...
                        <h1 className="text-4xl md:text-5xl font-bold mt-2">Energy <span className="bg-gradient-to-r from-amber-400 to-yellow-400 bg-clip-text text-transparent">Infrastructure</span></h1>
                        <p className="text-gray-400 mt-4 max-w-3xl text-sm leading-relaxed">Physical networks that produce, convert, move, store, and deliver energy across fossil, renewable, storage, and electricity systems.</p>
                    </motion.div>
                </div>
                ...
                <div className="flex overflow-x-auto gap-3 mb-12 pb-2" style={{ scrollbarWidth: 'none' }}>
                    {['layer-map','matrix','assets','constraints','storage','grid','resilience','scenarios'].map(h => (
                        <a key={h} href={'#'+h} className="px-4 py-2 whitespace-nowrap bg-white/5 border border-white/10 rounded-full hover:bg-amber-500/20 hover:text-amber-400 transition-all text-sm">{h.charAt(0).toUpperCase()+h.slice(1)}</a>
                    ))}
                </div>

                {/* 1. LAYER MAP */}
                <section id="layer-map" className="mb-16">
                    <h2 className="text-2xl font-bold mb-6 flex items-center gap-3"><Layers className="w-6 h-6 text-amber-400" /> Infrastructure Layer Map</h2>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        {INFRASTRUCTURE_LAYERS.map(l => {
                            const Icon = l.icon
                            const active = layer === l.id
                            return (
                                <button key={l.id} onClick={() => setLayer(active ? null : l.id)}
                ...
                <div className="mb-8">
                    <div className="flex flex-wrap gap-2">
                        {TECH_CATS.map(c => (
                            <button key={c.id} onClick={() => { setTech(c.id); setLayer(null) }}
                                className={`px-4 py-2 rounded-full text-sm font-medium transition-all border ${tech === c.id ? 'bg-amber-500/20 text-amber-400 border-amber-500/40' : 'bg-white/5 text-gray-400 border-white/10 hover:text-white'}`}>{c.label}</button>
                        ))}
                    </div>
                </div>
                ...
                <section id="matrix" className="mb-16">
                    <h2 className="text-2xl font-bold mb-6 flex items-center gap-3"><BarChart3 className="w-6 h-6 text-amber-400" /> Technology Coverage Matrix</h2>
                ...
                <section id="assets" className="mb-16">
                    <h2 className="text-2xl font-bold mb-6 flex items-center gap-3"><Factory className="w-6 h-6 text-amber-400" /> Infrastructure Asset Explorer <span className="text-sm font-normal text-gray-500 ml-2">({layerFiltered.length})</span></h2>
                    <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {layerFiltered.map(a => {
                            const Icon = a.icon
                            return (
                                <button key={a.id} onClick={() => setSelAsset(selAsset === a.id ? null : a.id)}
                                    ...
                                    {selAsset === a.id && (
                                        <div className="mt-3 pt-3 border-t border-white/10">
                                            ...
                                            <div className="flex flex-wrap gap-1 mb-2">
                                                {a.metrics.map(m => (
                                                    <span key={m} className="text-[9px] px-1.5 py-0.5 rounded bg-blue-500/10 text-blue-400 border border-blue-500/20">{m}</span>
                                                ))}
                                            </div>
                ...
                <section id="constraints" className="mb-16">
                    <h2 className="text-2xl font-bold mb-6 flex items-center gap-3"><AlertTriangle className="w-6 h-6 text-red-400" /> Infrastructure Constraints</h2>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        {CONSTRAINTS.map(c => {
                            const Icon = c.icon
                            ...
                            return (
                                <div key={c.label} className={`rounded-xl border p-4 ${sc}`}>
                                    <div className="flex items-center gap-2 mb-2"><Icon className="w-4 h-4" /><span className="text-sm font-bold">{c.label}</span></div>
                                    <p className="text-xs text-gray-400 leading-relaxed">{c.desc}</p>
                                    <div className="mt-2 text-[10px] uppercase tracking-wider font-bold text-gray-500">Severity: <span className={c.sev === 'high' ? 'text-red-400' : c.sev === 'medium' ? 'text-yellow-400' : 'text-gray-400'}>{c.sev}</span></div>
                                </div>
                            )
                        })}
                    </div>
                ...
                <section id="storage" className="mb-16">
                    <h2 className="text-2xl font-bold mb-6 flex items-center gap-3"><Battery className="w-6 h-6 text-purple-400" /> Storage & Buffering Comparison</h2>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        {STORAGE.map(s => {
                            const Icon = s.icon
                            return (
                ...
                <section id="grid" className="mb-16">
                    <h2 className="text-2xl font-bold mb-6 flex items-center gap-3"><Zap className="w-6 h-6 text-yellow-400" /> Grid & Integration</h2>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                        {GRID.map(g => (
                ...
                <section id="resilience" className="mb-16">
                    <h2 className="text-2xl font-bold mb-6 flex items-center gap-3"><Shield className="w-6 h-6 text-emerald-400" /> Resilience & Vulnerability</h2>
                        ...
                        {RESILIENCE.map((r, i) => (
                            <div key={r.dim} className="rounded-xl border border-white/10 bg-white/5 p-4">
                                <div className="flex items-center justify-between mb-3">
                                    ...
                                    <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${resScores[i] === 0 ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/20 text-red-400'}`}>
                    ...
                    <p className="text-[10px] text-gray-600 mt-3 italic">Scores are illustrative. Each dimension should be backed by documented data sources and methodology notes.</p>
                </section>

                {/* 8. SCENARIOS */}
                <section id="scenarios" className="mb-16">
                    <h2 className="text-2xl font-bold mb-6 flex items-center gap-3"><RefreshCw className="w-6 h-6 text-amber-400" /> Scenario Stress View</h2>
                    <p className="text-gray-400 text-sm mb-6 max-w-3xl">Illustrative scenarios showing how infrastructure stress moves through the physical system. Not a forecast.</p>
                    <div className="grid gap-4 sm:grid-cols-2">
                        {SCENARIOS.map(s => (
                            <button key={s.id} onClick={() => setSelScenario(selScenario === s.id ? null : s.id)}
                                ...
                                {selScenario === s.id && (
                                    <div className="mt-3 pt-3 border-t border-white/10">
                                        ...
                                        <p className="text-[9px] text-gray-600 mt-2 italic">Illustrative scenario &middot; Not a forecast &middot; Based on selected assumptions</p>
                                    </div>
                                )}
                            </button>
                        ))}
