...
import {
    ArrowLeft, Flame, TrendingUp, TrendingDown, Map as MapIcon, ShieldAlert,
    Thermometer, Route, BoxSelect, Cpu, Ship, Anchor, Crosshair, Radio, Calculator, BrainCircuit
} from 'lucide-react'

// --- MOCK DATA ---
const INTELLIGENCE_LAYERS = [
    { id: 'ais', name: 'Standard AIS', desc: 'Regular ship tracking', icon: Ship, color: 'text-blue-400' },
    { id: 'shadow', name: 'Shadow Mode', desc: 'AI-detected Dark Ships', icon: Crosshair, color: 'text-purple-400' },
    { id: 'thermal', name: 'Thermal Pulse', desc: 'Refinery heat signatures', icon: Thermometer, color: 'text-red-400' },
]
...
export default function OilAndGasIntelligence() {
    ...
    return (
        <main className={(simulationMode ? "min-h-screen pt-28 pb-24 transition-colors duration-500 bg-[#1a1100] border-amber-500/30 border-8" : "min-h-screen pt-28 pb-24 transition-colors duration-500 bg-gradient-to-br from-[#0a0a0c] to-black")}>
            <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                
                {/* --- HEADER --- */}
                <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div>
                        <Link href="/energy" className="inline-flex items-center gap-2 text-gray-400 hover:text-geo-gold transition-colors mb-6">
                        ...
                        <h1 className="text-4xl md:text-5xl font-bold mt-2 text-white">
                            Geopolitical <span className="bg-gradient-to-r from-amber-400 to-orange-500 bg-clip-text text-transparent">Hydrocarbon</span> Matrix
                        </h1>
                    </div>
                    ...
                    >
                        ...
                        <div className="text-left leading-tight">
                            <div className="text-xs font-mono opacity-70">Simulation Dock</div>
                            <div className="font-bold">{simulationMode ? 'ACTIVE' : 'STANDBY'}</div>
                        </div>
                    </button>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 flex flex-col gap-6">
                        ...
                        {/* Map Area */}
                        <div className={`relative h-[500px] rounded-2xl overflow-hidden border ${simulationMode ? 'border-amber-500/50' : 'border-white/10'} bg-black/50 p-1`}>
                            {/* Fake Map Background */}
                        ...
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 relative overflow-hidden">
                            <h3 className="text-xl font-bold flex items-center gap-2 mb-6">
                                <TrendingUp className="text-amber-500 w-5 h-5" /> 7-Day Predictive Price Engine
                            </h3>
                            <div className="h-48 flex items-end justify-between gap-2 border-b border-l border-white/10 pl-2 pb-2 relative">
                                ...
                                {[40, 45, 60, 50, 70, 65, 80].map((h, i) => (
                                    <div key={i} className="relative w-full flex justify-center group h-full items-end">
                                        <div style={{ height: (h + "%") }} className="w-2/3 bg-blue-500/50 rounded-t-sm transition-all" />
                                        <div style={{ height: (h + 5 + "%") }} className="w-1 bg-geo-gold absolute rounded-t-sm z-10 shadow-[0_0_8px_rgba(255,215,0,0.5)]" />
                                        {/* Simulation Line */}
                                        {simulationMode && (
                                            <div style={{ height: (h + (i*10) + "%") }} className="w-1 bg-orange-500 absolute -right-1 rounded-t-sm z-20 border-l border-dashed border-black" />
                                        )}
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="flex gap-4 mt-6 text-xs font-medium pl-2">
                                <div className="flex items-center gap-2 text-gray-400"><div className="w-3 h-3 bg-blue-500 border border-blue-400 rounded-sm" /> BASELINE CURRENT</div>
                                <div className="flex items-center gap-2 text-geo-gold"><div className="w-3 h-3 bg-geo-gold rounded-full" /> AI PREDICTED</div>
                                {simulationMode && (
                                    <div className="flex items-center gap-2 text-orange-400"><div className="w-3 h-3 bg-orange-500 rounded-sm" /> SIMULATED IMPACT</div>
                                )}
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
                            ...
                            <p className="text-sm text-gray-400 mb-6">Real-time Last-Mile delivery risk for regional arbitrage calculations.</p>
                            
                            <div className="space-y-4">
                                <div className="bg-black/30 rounded-xl p-4 border border-white/5 relative overflow-hidden">
                                    <div className="absolute top-0 right-0 w-1 h-full bg-red-500" />
                                    <div className="flex justify-between items-start mb-2">
                                        <span className="font-medium text-red-100">NE India Corridor</span>
                                ...
                                <div className="bg-black/30 rounded-xl p-4 border border-white/5 relative overflow-hidden">
                                    ...
                                    <div className="flex justify-between items-start mb-2">
                                        <span className="font-medium text-orange-100">Euro ARA Hubs</span>
                                        <span className="text-2xl font-black text-orange-500">62</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-xs text-gray-500">
                                ...
                                <div className="bg-black/30 rounded-xl p-4 border border-white/5 relative overflow-hidden">
                                    ...
                                    <div className="flex justify-between items-start mb-2">
                                        <span className="font-medium text-emerald-100">US Gulf Coast</span>
                                        <span className="text-2xl font-black text-emerald-500">21</span>
                                    </div>
                                    <div className="flex items-center gap-2 text-xs text-gray-500">
                        ...
                        <div className="bg-gradient-to-br from-gray-900 to-black border border-white/10 rounded-2xl p-6">
                            <h3 className="font-bold flex items-center gap-2 mb-4 text-white">
                                <Calculator className="w-5 h-5 text-geo-gold" /> Industry Converter
                            </h3>
                            <div className="flex flex-col gap-4">
                                <div className="flex items-center gap-2">
                                    <input type="number" placeholder="1000" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-white outline-none focus:border-geo-gold" />
                                ...
                                <div className="flex items-center gap-2">
                                    <input type="number" readOnly value="158.98" className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-2 text-gray-300 outline-none" />
                                    <select className="bg-white/5 border border-white/10 rounded-lg px-2 py-2 text-white outline-none">
                                ...
                                <a href="https://www.rigzone.com/calculator/" target="_blank" rel="noreferrer" className="text-[10px] text-center text-gray-500 hover:text-geo-gold mt-2 transition-colors">
                        ...
                        <div className="bg-blue-950/20 border border-blue-500/20 rounded-2xl p-6 relative overflow-hidden group">
                            ...
                            <div className="space-y-3 relative z-10">
                                <div className="border border-white/5 bg-black/40 rounded-lg p-3">
                                    <p className="text-xs text-blue-200 font-medium mb-1">OPEC+ Quota Compliance</p>
                                    <p className="text-[11px] text-gray-400 line-clamp-2">Satellite detection shows 4 member states exceeding agreed export limits, likely triggering a response from Riyadh next week.</p>
                                </div>
                                <div className="border border-white/5 bg-black/40 rounded-lg p-3">
                                    <p className="text-xs text-blue-200 font-medium mb-1">Strategic Petroleum Reserve</p>
                                    <p className="text-[11px] text-gray-400 line-clamp-2">US repurchases slowing due to target price misses; arbitrage windows for Gulf refiners widening.</p>
                                </div>
                            </div>
                        </div>
